...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # as shown below.
        self.activation_function = lambda x : 1 / (1 + np.exp(-x))  # Implemented Sigmoid function
    ...
    def forward_pass_train(self, X):
        ...
        ### Forward pass ###
        # Hidden layer
        # Input to hidden layer is Sum(Weights * X)
        hidden_inputs = np.dot(X, weights_input_to_hidden) # signals into hidden layer
        print(type(hidden_inputs), hidden_inputs.shape)
